...
jclass cls = (*jenv)->FindClass(jenv,
		"org/sosy_lab/java_smt/solvers/mathsat5/Mathsat5NativeApi$AllSatModelCallback");
if (cls == NULL) {
	goto out;
}
...
DEFINE_FUNC(object, 1named_1list_1from_1smtlib2) WITH_TWO_ARGS(jenv, string)
    ...
    jclass cls = (*jenv)->FindClass(jenv,
        "org/sosy_lab/java_smt/solvers/mathsat5/Mathsat5NativeApi$NamedTermsWrapper");
    if (cls == NULL) {
